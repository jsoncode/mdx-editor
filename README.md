...
| 媒体转码 | 通过 **FFmpeg**（可选内置或用户配置路径）将 WMA、WMV、AVI 等格式转码为 MP4/M4A 后内联播放 |
...
- **FFmpeg（可选）**：默认安装包**不包含** FFmpeg。若需预览 WMA、WMV 等格式，请在应用 **设置 → 媒体预览** 中配置本机 FFmpeg 路径，或确保 `ffmpeg` 已在系统 PATH 中；也可使用 `npm run pack:ffmpeg` 构建自带 FFmpeg 的安装包
...
1. **设置中配置的路径**（设置 → 媒体预览 → FFmpeg 路径）
2. **安装包内置 sidecar**（仅在使用 `pack:ffmpeg` 系列命令打包时包含）
3. **系统 PATH** 中的 `ffmpeg` / `ffmpeg.exe`

#### 默认打包（不含 FFmpeg，推荐）

安装包体积更小。用户自行安装 FFmpeg 并在设置中指定路径，或依赖系统 PATH 即可使用转码能力：
...
| FFmpeg 路径 | 媒体转码使用的 FFmpeg 可执行文件；留空则使用内置 sidecar 或系统 PATH | 空 |
...
- 填写或浏览选择本机 `ffmpeg.exe`（Windows）路径
- 点击 **测试 FFmpeg** 验证是否可用
- 留空时依次尝试：安装包内置 FFmpeg（若有）→ 系统 PATH
...
**Q：WMA / WMV 无法在预览中播放？**  
A：WebView2 不支持这些格式的原生解码。请在 **设置 → 媒体预览** 配置 FFmpeg 路径，或确保系统 PATH 中有 `ffmpeg`；也可使用 `npm run pack:ffmpeg` 构建自带 FFmpeg 的安装包。配置后首次播放会转码，稍等片刻即可。

**Q：如何确认 FFmpeg 是否可用？**  
A：打开 **设置 → 媒体预览**，点击 **测试 FFmpeg**。成功会显示 FFmpeg 版本信息；失败请检查路径是否正确、文件是否存在。

**Q：默认安装包为什么不带 FFmpeg？**  
A：FFmpeg 体积较大（约 200MB），多数用户只需预览 MP3/MP4 等常见格式。需要转码能力的用户可在设置中指定本机 FFmpeg，发布者可选 `npm run pack:ffmpeg` 构建内置版。
